/**
 * University names database
 * Extracted from Kaggle world universities dataset + popular Indian universities
 */

let cachedUniversities = null;

// Popular Indian universities to add at priority positions
...
const UNIVERSITIES = [
  ...
  'Akrofi-Christaller Institute of Theeology',
  'Aksum University',
  ...
  'Dubna International University for Nature',
  'Duke University',
  ...
  'Evangelische Fachhochschule für Religionspädagogik',
  ...
  'Hogeschool voor Wetenschap en Kunst (VLEKHO)',
  ...
  'International University of Engineering',
  ...
  'NGO Vladikavkaz Institute of Economics',
  ...
  'Teacher Training College of Revalidation',
  'Teachers College',
  ...
  'Technological Education Institute of Halkida',
  ...
  'Trinity International University (Excel)',
 ...
 * @returns {string[]} Array of university names
 */
export function getUniversities() {
  ...
  const allUniversities = [...INDIAN_UNIVERSITIES, ...UNIVERSITIES];
  const uniqueUniversities = [...new Set(allUniversities)].sort();

  // Prioritize Stony Brook at the very top
  const stonyBrook = uniqueUniversities.find(uni =>
    uni.toLowerCase().includes('stony brook')
  );

  if (stonyBrook) {
    const filtered = uniqueUniversities.filter(uni => uni !== stonyBrook);
    cachedUniversities = [stonyBrook, ...filtered];
  } else {
    cachedUniversities = uniqueUniversities;
  }
 ...
 * @param {number} minLength - Minimum query length to show results
 * @returns {string[]} Filtered universities
 */
export function searchUniversities(query, minLength = 2) {
  ...
  // Prioritize Stony Brook at the top if it matches
  const stonyBrookMatch = matches.find(uni =>
    uni.toLowerCase().includes('stony brook')
  );

  if (stonyBrookMatch) {
    // Move Stony Brook to the top
    return [stonyBrookMatch, ...matches.filter(uni => uni !== stonyBrookMatch)];
  }
