...
test.describe('YeetCode Electron App', () => {
  ...
  test('should launch Electron app successfully', async () => {
    ...
    await expect(
      page.locator('h2:has-text("Welcome to YeetCode! 🚀")')
    ).toBeVisible();
  });
  ...
  test('should navigate through onboarding interface', async () => {
    ...
    await page.fill(
      'input[placeholder="Your LeetCode username"]',
      'electronuser123'
    );

    // Verify form elements are present and functional
    await expect(
      page.locator('input[placeholder="Enter your first name"]')
    ).toHaveValue('Electron User');
    await expect(
      page.locator('input[placeholder="Your LeetCode username"]')
    ).toHaveValue('electronuser123');
  ...
  test('should display development skip option', async () => {
    ...
    await expect(
      page.locator('button:has-text("Skip Group Setup")')
    ).toBeVisible();
  ...
  test('should handle desktop app workflow using dev helpers', async () => {
    ...
    await page.waitForSelector('text=Group: DEV-SKIP');
    await expect(
      page.locator('text=User: Desktop User (desktopuser123)')
    ).toBeVisible();
  ...
  test('should join group 43837 and display leaderboard', async () => {
    ...
    await expect(
      page.locator('text=User: E2E Test User (testuser)')
    ).toBeVisible();
    ...
    await page.waitForSelector('h2:has-text("Join or Create a Group")');
    await expect(
      page.locator('h2:has-text("Join or Create a Group")')
    ).toBeVisible();
  });
});
